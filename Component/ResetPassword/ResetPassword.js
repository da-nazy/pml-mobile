...
const [confirmPwd,setConfirmPwd]=useState({
    ...
    <View>
        ...
        <View>
          <InputComp mode="outlined" right={<TextInput.Icon name="eye"  onPress={()=>setCurrentPwd({...currentPwd,cpSecure:!currentPwd.cpSecure})}/>}  label="New Password" placeholder="Input value"  style={style.name} error={currentPwd.cpError} secureText={currentPwd.cpSecure} setText={(e)=>{setCurrentPwd({...currentPwd,currentPassword:e})}}/>
          {currentPwd.cpError?(<Text style={{marginLeft:25,color:'red'}}>Invalid input.</Text>):null}
            </View>
            <View>
          <InputComp mode="outlined" right={<TextInput.Icon name="eye"  onPress={()=>setConfirmPwd({...confirmPwd,cpSecure:!confirmPwd.cpSecure})}/>}  label="Confirm New Password" placeholder="Input value"  style={style.name} error={confirmPwd.cpError} secureText={confirmPwd.cpSecure} setText={(e)=>{setConfirmPwd({...confirmPwd,confirmPwd:e})}}/>
